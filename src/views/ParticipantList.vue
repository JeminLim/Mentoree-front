                    ...
                    <div class="card-body">
                        <table class="w-100">
                            ...
                            <tr v-for="(applicant, index) in getApplicants" v-bind:key="applicant.id">
                                ...
                                <td width="15%">
                                    <button class="btn btn-primary" @click="accept(applicant, index)">승낙</button>
                                    <button class="btn btn-danger" @click="reject(applicant, index)">거절</button>
                                </td>
                            </tr>
                            </tbody>
                        </table>
...
export default {
    ...
    methods: {
        accept(applicant, index) {
            if(this.currentNumMember >= this.programInfo.maxMember) {
                ...
                axios.post('/mentoring-service/api/programs/' + this.programInfo.id + '/applicants/accept' , targetData)
                    .then(() => {
        ...
        reject(applicant, index) {
            ...
            axios.post('/mentoring-service/api/programs/' + this.programInfo.id + '/applicants/reject' , targetData)
                .then(() => {
                    this.applicants.splice(index, 1);
                    alert("거절 완료되었습니다.");
                }).catch(err => {
    ...
    created() {
        ...
        axios.get('/mentoring-service/api/programs/' + programId + '/applicants')
        .then(res => {
