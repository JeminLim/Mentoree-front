<template>
    <div>
        ...
        <!-- 추천 프로그램이 없을 경우 -->
        <div v-if="!this.$store.state.user.isLogin" class="col-md-12 text-center">
            <span>로그인이 필요한 서비스입니다.</span>
        </div>
        <div v-else-if="!this.$store.state.user.userInfo.interests.length > 0" class="col-md-12 text-center">
            <span>관심분야를 설정해주세요.</span>
        </div>
        <div v-else id="recommendArea" class="row">
...
export default {
    ...
    methods: {
        reloadRecommendProgram() {
            axios.get('/mentoring-service/api/programs/recommendations/list', { params:{
                page : this.page
            }}).then(res => {
                if (res.data.recommendProgramList != null) {
                    this.page += 1;
                    this.programs = [];
                    for(var i = 0; i < res.data.recommendProgramList.length; i++) {
                        this.programs.push(res.data.recommendProgramList[i])
                    }
    ...
    beforeMount() {
        if(this.$store.state.user.userInfo.interests != null) {
            if(this.$store.state.user.isLogin === true && 
                this.$store.state.user.userInfo.interests.length > 0) {
